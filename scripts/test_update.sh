...
if ! grep -wq -- "$TEXT_TO_FIND" "$CONTROLLER_DIR/NOTICE"; then
    echo "Unable to find $TEXT_TO_FIND in the 'NOTICE' file of $CONTROLLER_NAME. Adding $TEXT_TO_FIND in controller-bootstrap/templates/NOTICE.tpl file ..."
...
if ! grep -wq -- "$TEXT_TO_FIND" "$CONTROLLER_DIR/NOTICE"; then
    echo "Unable to find "$TEXT_TO_FIND" in the 'NOTICE' file. Failed to update the project description files in $CONTROLLER_NAME. Exiting"
    rm -rf ${OUTPUT_DIR}
    exit 1
else
